...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the daily production XGBoost pipeline and send an operational email when configured."
    )
    ...
    try:
        result = run_daily_pipeline_with_notifications(
            ...
            forecasts_root=args.forecast_output_root,
            monitoring_root=args.monitoring_output_root,
            operations_root=args.operations_output_root,
            max_source_age_days=args.max_source_age_days,
            skip_source_refresh=args.skip_source_refresh,
        )
    except Exception:
        logging.exception("SPD XGBoost production pipeline failed.")
        return 1
    logging.info(
        "Production run %s (%s)",
        result["run_dir"],
        "idempotent" if result["idempotent"] else "created",
    )
    return 0
